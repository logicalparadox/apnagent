...
Message.prototype.contentAvailable = function(ca) {
  this.settings['content-available'] = ca ? 1 : undefined;
  return this;
};
...
Message.prototype.serialize = function() {
  ...
  var payload = {};
  var SE = errors.MessageSerializationError;
  ...
  if (len > 256 && this.aps.body) {
    var over = len - 256;
    var bodyLen = Buffer.byteLength(this.aps.body, enc);
    var body = bodyLen <= over ? null : util.trim(this.aps.body, bodyLen - over);

    if (!body) throw new SE('Message too long.', null, ssf);
  ...
  res.identifier = this._agent ? this._agent.nextId() : null;
  res.payload = payload;
  return res;
};
