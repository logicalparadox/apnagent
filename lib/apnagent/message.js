 ...
 * ### .actions(array)
 *
 * Add custom actions which iOS will display with the alert.
 *
 * ```js
 * var action = new Action().set('title', 'delete');
 * msg.actions([action]);
 * ```
 *
 * @param {Array} array of Action
 * @returns {this} for chaining
 * @api public
 * @name actions
 */

 Message.prototype.actions = function (actions) {
   if(!Array.isArray(actions)) return this;
   this.alert('actions', actions.map(function(action){ return action.detail; }))
   return this;
 }
