...
function Base() {
  ...
  var self = this;
  var cache, decoder, encoder, queue;
  ...
  decoder.stream(8).on('readable', function () {
    ...
    var err = new errors.GatewayMessageError(message, obj);
    var cached = self.cache.get(obj.identifier);
    var msg = null;
...
Base.prototype.nextId = function() {
  ...
  if (i > INT32) i = this.meta.lastId = 0;
  return i;
};
...
Base.prototype.send = function(msg, cb) {
  ...
  var self = this;
  var codecStr = msg.meta.codec;
  var codec = codecs.getId('gateway ' + codecStr);
  var json;
